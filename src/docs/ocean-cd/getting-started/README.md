...
1. Fill in your cluster ID. The cluster ID must be unique, have up to 30 alphanumeric characters, and not contain spaces. The Ocean CD cluster ID is a logical identifier for your cluster. You can choose any ID you want, and it is not coupled to the Ocean cluster ID (o-xxxxxx). Ocean CD can run on clusters that are not managed by Ocean.
