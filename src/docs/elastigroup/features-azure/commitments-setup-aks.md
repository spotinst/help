...
*  `utilizeReservedInstances`: If there are free reserved instances within the AWS account, Elastigroup will utilize them before launching spot instances. The initial default value is inherited from the identical attribute at the cluster level.
*  `utilizeSavingPlans`: If savings plan commitments have available capacity, Elastigroup will utilize them alongside existing reserved instances to maximize cost efficiency. The initial default value is inherited from the identical attribute at the cluster level.
