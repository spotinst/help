...
It is important to determine how you wish Eco to operate. If RI sharing is not turned on, Eco will need one Spot org per linked account, meaning either one CUR per linked-account or one exported CSV gzip (defined below) per org. 

If customer has RI sharing turned on specifically for the group of linked accounts, and is okay with Cost Specialists managing it cross-account; then one exported CSV gzip (below) per org is fine.
