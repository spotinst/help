...
Whether you are rolling out changes to your entire Ocean cluster, a particular virtual node group (VNG), or specific nodes, Ocean offers the flexibility to divide the roll into batches based on batch sizes of your choosing. For example, you opt for the default batch size of 20%. In this case, Ocean will divide the roll into five batches.
...
2.	Ocean starts with the first batch, replacing each node in such a way that ensures the successful accommodation of the workloads on the new nodes. Ocean's autoscaler takes into consideration all relevant constraints in place before the roll.
3.	When all nodes in a batch complete processing and at least 50% of them have successful replacement, then Ocean starts to work on the next batch. In the Spot API, you can configure the percentage using the batchMinHealthyPercentage parameter, explained below.
...
*   Respect Restrict Scale Down (RSD) during Roll: Rolls do not consider the restrict-scale-down label. Ocean will replace a node even if a task or pod uses this label. Ocean's autoscaler takes all configured constraints into consideration before the roll.
...
*   Batch Size Healthy Percentage: indicates the minimum percentage of healthy instances in a single batch.
    If the number of healthy instances in a single batch is below this percentage, the roll will fail. The range is 1-100, and if the parameter value is null, the default value will be 50%. Ocean considers instances not replaced due to PDB as healthy.
...
*   COMPLETED: All nodes have been processed, and at least 50% of them have been successfully replaced.

>**Tip:** In the Console, a specific batch may appear with a Pending state. This means that even though the roll process has started, that batch has not yet started to replace its nodes.  
...
You can schedule a roll in the Create Cluster or Update Cluster [Spot API](https://docs.spot.io/api/#tag/Ocean-AKS/operation/oceanAKSV2ClusterUpdate) using a cron expression. This enables you to easily run the roll during off hours.
...
Similarly, the Spot API also lets you roll one or more node pools without having to roll the entire cluster. Do this by specifying a list of node Pool IDs or a specific Node Pool ID.
...
>**Note:** To stop a roll while it is running, click the **Stop Roll** button on the right of the screen, and then click **Stop Roll** in the confirmation box.

##  Create or Edit a Roll Schedule 

>**Note:** You can schedule either cluster or Virtual Node Group rolls. You cannot schedule Node Pools rolls.
