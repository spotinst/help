...
6. Select one of the **Restart replicas** options:
   ...
   * No restart.
7. Click the **Set the resources percentage change** down arrow to apply the recommendation, and set the CPU and Memory percentage thresholds. This is the minimum percentage change from the current request for applying a recommendation. If the right-sizing recommendations exceed the percentage threshold for either resource (CPU or Memory), they will still be applied to prevent unnecessary pod deletion. However, if a deletion occurs for one of the resources, we can already apply the recommendation to the other. 
