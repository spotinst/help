...
- Ensure frequent scaling pods won't wait for instances to launch (see Headroom section for more details).
...
During suspension hours, Ocean Autoscaler does not scale down instances. Suspension hours override the restricted scale-down setting, which prevents scale-down for time frames when there is no Ocean activity, such as bin packing, reverting to lower-cost nodes, or reverting to reserved instances.
